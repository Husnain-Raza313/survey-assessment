...
const HyperlinkInput: React.FC<HyperlinkInputProps> = ({ hyperlink }) => {
  return (
    <div
      className="px-2 border-2 border-gray-300 bg-white rounded-lg flex items-center justify-between 2xl:w-[75%]"
      aria-label={`Hyperlink: ${hyperlink}`}
    >
      <input
        type="text"
        value={hyperlink}
        ...
        readOnly
        aria-label={`Hyperlink field containing: ${hyperlink}`}
      />
      <div aria-label="Open hyperlink">
        <svg
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 111.07 122.88"
          className="h-[20px] text-blue-700"
          fill="currentColor"
          aria-hidden="true"
        >
          <g>
